...
bing_meta_file = os.path.join(os.path.dirname(__file__), "bing_data.json")
...
def scrape_bing_item(item_soup, main_link, wallpapers_dir, headers):
    ...
    description = ' '.join([p.text.strip() for p in item_soup.select('div.position-relative p')]).strip()
    date = item_soup.select_one('time')['datetime']
    tags = [tag.text.strip() for tag in item_soup.select('div.tag-list a.tag-list__item')]
    ...
    local_image_path = os.path.join(wallpapers_dir, image_filename)
